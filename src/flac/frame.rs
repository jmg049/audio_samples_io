...
mod tests {
    ...
    fn test_encode_decode_frame_mono_constant() {
        ...
        assert!(
            channels[0].iter().all(|&s| s == 0),
            "silence should decode to zeros"
        );
    }
    ...
    fn test_encode_decode_frame_large_block() {
        ...
        let (frame, consumed) =
            decode_frame(&frame_bytes, 48000, 16, 1).expect("decode large block");
